...
const TransactionCard = ({ tx }: { tx: any }): JSX.Element => {
  ...
  return (
    <div
      className={`${bgColor} m-6 flex flex-1 2xl:min-w-[350px]
            2xl:max-w-[500px]
            sm:min-w-[270px]
            sm:max-w-[300px]
            min-w-full
...
const Transaction = (): JSX.Element => {
  const { getAllTransactions, currentAccount } =
  ...
  useEffect(() => {
    const call = async () => {
      if (currentAccount) {
        ...
        const transaction = await getTransactionsForConnectedAccount(
          transactions,
          currentAccount
        );
        ...
        {transactions ? (
          <div className="flex flex-wrap justify-center items-center mt-10">
            {transactions.map((tx: any, index: any) => {
              return <TransactionCard key={index} tx={tx} />;
            })}
          </div>
        ) : loading ? (
          <TransactionLoader />
        ) : (
          <h3 className="text-white text-center mt-4">No transactions</h3>
        )}
      </div>
    </div>
  );
